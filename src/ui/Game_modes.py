...
class GameModes:
    def __init__(self, screen, save_manager):
        ...
        # Lista para armazenar os retângulos dos botões de exclusão e seus nomes de save
        self.delete_buttons_rects = []
        self.delete_font = pygame.font.SysFont(None, 24, bold=True) # Fonte para o 'X'
    ...
    def _handle_load_game_click(self, pos):
        ...
        # Converte a posição do clique para as coordenadas internas da temp_surface
        display_area_start_y = 100
        click_x_temp = pos[0]
        click_y_temp = pos[1] - (display_area_start_y - self.scroll_offset)
        click_pos_temp = (click_x_temp, click_y_temp)

        # Verifica cliques nos botões de exclusão PRIMEIRO
        for delete_rect_temp, save_name_to_delete in self.delete_buttons_rects:
            # Calcula a posição Y real do botão de exclusão na tela para verificar visibilidade
            ...
            if (delete_rect_temp.collidepoint(click_pos_temp) and
                ...
                actual_delete_button_screen_y + delete_rect_temp.height <= (self.screen_height - 80)): # -80 para margem do botão "Voltar"
                
                self.save_manager.delete_save(save_name_to_delete)
                self.saved_games = self.save_manager.load_all_saves() # Recarrega os saves
                self.scroll_offset = 0 # Reseta a rolagem
                return # Sai após lidar com a exclusão

        # Lógica original para carregar o jogo (se um item de save em si for clicado)
        # Esta parte só será alcançada se nenhum botão de exclusão foi clicado
        y_offset = 100 - self.scroll_offset # Posição inicial da lista ajustada pela rolagem
        for name, data in self.saved_games.items():
            button_rect = pygame.Rect(
                (self.screen_width - 400) // 2, y_offset, 400, self.item_height - 10
    ...
    def _draw_load_game_menu(self):
        ...
        self.delete_buttons_rects = [] # Limpa a lista para este ciclo de desenho
        ...
        if not self.saved_games:
            ...
            for name, data in self.saved_games.items():
                ...
                # Desenha o botão de exclusão na temp_surface
                ...
                # Armazena o retângulo (relativo à temp_surface) e o nome do save
